...
LINK_OUTPUT := $(BUILD_DIR)/mmul.link.xclbin
PACKAGE_OUT = ./package.$(TARGET)

VPP_PFLAGS := 
CMD_ARGS = -x $(BUILD_DIR)/mmul.xclbin
...
CXXFLAGS += -I$(HOME)/Documents/GEMM_opt/common/includes/cmdparser
CXXFLAGS += -I$(HOME)/Documents/GEMM_opt/common/includes/logger
HOST_SRCS += $(HOME)/Documents/GEMM_opt/common/includes/cmdparser/cmdlineparser.cpp $(HOME)/Documents/GEMM_opt/common/includes/logger/logger.cpp ./src/host.cpp 
...
all: check-platform check-device check-vitis $(EXECUTABLE) $(BUILD_DIR)/mmul.xclbin emconfig

.PHONY: host
host: $(EXECUTABLE)

.PHONY: build
build: check-vitis check-device $(BUILD_DIR)/mmul.xclbin

.PHONY: xclbin
xclbin: build

############################## Setting Rules for Binary Containers (Building Kernels) ##############################
$(TEMP_DIR)/mmul.xo: src/mmul.cpp
	mkdir -p $(TEMP_DIR)
	v++ -c $(VPP_FLAGS) -t $(TARGET) --platform $(PLATFORM) -k mmul --temp_dir $(TEMP_DIR)  -I'$(<D)' -o'$@' '$<'
...
ifeq ($(TARGET),$(filter $(TARGET),hw_emu))
	...
	XCL_EMULATION_MODE=$(TARGET) XRT_INI_PATH=./xrt.ini $(EXECUTABLE) $(CMD_ARGS)
else
	$(EXECUTABLE) $(CMD_ARGS)
endif
